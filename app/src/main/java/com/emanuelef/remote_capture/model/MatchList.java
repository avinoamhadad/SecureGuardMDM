...
public class MatchList {
    ...
    public void reload() {
        ...
        //AppLogger.d(TAG, serialized);
        //LogUtil.logToFile(mPrefName+"res="+ serialized);
        if(!serialized.isEmpty()) {
            fromJson(serialized);

            if(mMigration) {
                AppLogger.i(TAG, "Migration completed");
                save();
                mMigration = false;
            }
        } else
           ...
           // for(JSONObject el: ruleArray) {
                ...
                try {
                    ...
                    if(typeStr.equals("ROOT_DOMAIN")) {
                        AppLogger.i(TAG, String.format("ROOT_DOMAIN %s migrated", val));
                        type = RuleType.HOST;
                        mMigration = true;
                    } else {
                        e.printStackTrace();
                ...
                if(type == RuleType.APP) {
                    ...
                    try {
                        ...
                        if(app != null) {
                            ...
                            AppLogger.i(TAG, String.format("UID %d resolved to package %s", uid, val));
                            mMigration = true;
                        } else {
                            AppLogger.w(TAG, "Ignoring unknown UID " + uid);
                            continue;
                        }
                    ...
                    if((app != null) && !app.getPackageName().equals(val)) {
                        AppLogger.i(TAG, "The UID " + app.getUid() + " mapping has changed from " + val + " to " + app.getPackageName());
                        val = app.getPackageName();
                        mMigration = true;
                    }
                }
...
*/
   ...
   /* private static class Serializer implements JsonSerializer<MatchList> {
    ...
    public boolean addApp(int uid) {
        AppDescriptor app = mResolver.getAppByUid(uid, 0);
        if(app == null) {
            AppLogger.e(TAG, "could not resolve UID " + uid);
            return false;
        }
    ...
    public void removeRule(Rule rule) {
        ...
        if(rule.getType() == RuleType.APP) {
            ...
            if(uid != Utils.UID_NO_FILTER) {
                ...
                AppLogger.w(TAG, "removeRule: no uid found for package " + val);
        } else if (rule.getType() == RuleType.IP) {
            if ((val.indexOf('/') >= 0) && !removeCidr(val))
                AppLogger.w(TAG, "removeRule: removing CIDR failed for " + val);
        }
    ...
    public String toJson(boolean pretty_print) {
      /*  GsonBuilder builder = new GsonBuilder().registerTypeAdapter(getClass(), new Serializer());
        ...
        //AppLogger.d(TAG, "toJson: " + serialized);
        try {
            if (pretty_print)
...
/*
    ...
    public ListDescriptor toListDescriptor() {
        ...
        while(it.hasNext()) {
            ...
            if(tp.equals(MatchList.RuleType.HOST))
                ...
                AppLogger.w(TAG, "ListDescriptor does not support RuleType " + tp.name());
        }
    ...
    public boolean uidMappingChanged(String pkg) {
        ...
        if((old_uid != null) && ((app == null) || (app.getUid() != old_uid))) {
            AppLogger.i(TAG, "Remove old UID mapping of " + pkg + ": " + old_uid);
        ...
        if((old_uid == null) && (app != null)) {
            ...
            AppLogger.i(TAG, "Add UID mapping of " + pkg + ": " + new_uid);
