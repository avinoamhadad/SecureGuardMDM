   ...
   // @Override
    ...
    private void openDb() {
       /* try {
            ...
            AppLogger.d(TAG, "Country DB loaded: " + mCountryReader.getMetadata());

            mAsnReader = new Reader(getAsnFile(mContext));
            AppLogger.d(TAG, "ASN DB loaded: " + mAsnReader.getMetadata());
        } catch (IOException e) {
            AppLogger.i(TAG, "Geolocation is not available");
        }*/
    }
...
/*
    ...
    private static boolean downloadAndUnzip(Context ctx, String label, String url, File dst) throws IOException {
        ...
        if(!rv) {
            AppLogger.w(TAG, "Could not download " + label + " db from " +  url);
            return false;
        }
        ...
        try{
            ...
            if(!Utils.ungzip(is, dst.getAbsolutePath())) {
                AppLogger.w(TAG, "ungzip of " + tmp_file + " failed");
                return false;
            }
