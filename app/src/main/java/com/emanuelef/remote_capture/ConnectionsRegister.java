...
public class ConnectionsRegister {
    ...
    public synchronized void newConnections(ConnectionDescriptor[] conns) {
        ...
        //AppLogger.d(TAG, "newConnections[" + mNumItems + "/" + mSize +"]: insert " + conns.length +
        ...
        for(ConnectionDescriptor conn: conns) {
            ...
            //AppLogger.d(TAG, "IP geolocation: IP=" + conn.dst_ip + " -> country=" + conn.country + ", ASN: " + conn.asn);
    ...
    public synchronized void connectionsUpdates(ConnectionUpdate[] updates) {
        ...
        AppLogger.d(TAG, "connectionsUpdates: items=" + mCurItems + ", first_id=" + first_id + ", last_id=" + last_id);
        ...
        for(ConnectionUpdate update: updates) {
            ...
            if((id >= first_id) && (id <= last_id)) {
                ...
                //AppLogger.d(TAG, "update " + update.incr_id + " -> " + update.update_type);
    ...
    public synchronized void addListener(ConnectionsListener listener) {
        ...
        AppLogger.d(TAG, "(add) new connections listeners size: " + mListeners.size());
    }

    public synchronized void removeListener(ConnectionsListener listener) {
        mListeners.remove(listener);

        AppLogger.d(TAG, "(remove) new connections listeners size: " + mListeners.size());
    }
    ...
    public synchronized void releasePayloadMemory() {
        AppLogger.i(TAG, "releaseFullPayloadMemory called");
