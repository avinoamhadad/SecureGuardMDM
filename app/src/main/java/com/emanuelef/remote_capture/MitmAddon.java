...
public class MitmAddon {
    ...
    private static class ReplyHandler extends Handler {
        ...
        public void handleMessage(@NonNull Message msg) {
            AppLogger.d(TAG, "Message: " + msg.what);
       ...
       /* if(!mContext.bindService(intent, mConnection, Context.BIND_AUTO_CREATE |
         ...
         if(!mContext.bindService(intent, mConnection, 513 | extra_flags)) {
            try {
                ...
                AppLogger.w(TAG, "unbindService failed");
            }
            mService = null;
            return false;
        }
    ...
    public void disconnect() {
        if(mService != null) {
            AppLogger.i(TAG, "Unbinding service...");
            try {
                mContext.unbindService(mConnection);
            } catch (IllegalArgumentException ignored) {
                AppLogger.w(TAG, "unbindService failed");
    ...
    public boolean stopProxy() {
        if(mService == null) {
            AppLogger.i(TAG, "Not connected, postponing stop message");
            mStopRequested = true;
            return true;
        }

        AppLogger.i(TAG, "Send stop message");
    ...
    public boolean disableDoze() {
        ...
        AppLogger.i(TAG, "Send disable doze");
