...
public class AppsResolver {
    ...
    public static AppDescriptor resolveInstalledApp(PackageManager pm, String packageName, int pm_flags, boolean warn_not_found) {
        ...
        try {
            ...
            if(warn_not_found)
                AppLogger.w(TAG, "could not retrieve package: " + packageName);
            return null;
        }
    ...
    public @Nullable AppDescriptor getAppByUid(int uid, int pm_flags) {
        ...
        if((packages == null) || (packages.length < 1)) {
            AppLogger.w(TAG, "could not retrieve package: uid=" + uid);
            return null;
        }
        ...
        if(!mFallbackToGlobalResolution && CaptureService.isCapturingAsRoot()) {
            try {
                ...
                AppLogger.w(TAG, "getPackageInfoAsUser call fails, falling back to standard resolution");
                e.printStackTrace();
    ...
    public int getUid(String package_name) {
        ...
        if(!package_name.contains(".")) {
            ...
            try {
                ...
                AppLogger.w(TAG, "Could not retrieve package " + package_name);
                //e.printStackTrace();
            }
        }
