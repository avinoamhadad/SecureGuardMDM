...
public class HTTPServer implements PcapDumper, Runnable {
    ...
    private static class ClientHandler implements Runnable {
        ...
        private void close(String error) {
            ...
            if(error != null) {
                AppLogger.i(TAG, "Client error: " + error);
                mHasError = true;
        ...
        public void run() {
            ...
            try {
                ...
                AppLogger.d(TAG, "Request headers end at " + req_size);
                //AppLogger.d(TAG, "Req: " + new String(buf, 0, req_size, StandardCharsets.UTF_8));
                ...
                try(BufferedReader reader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(buf, 0, req_size)))) {
                    ...
                    if(url.equals("/")) {
                        ...
                        AppLogger.d(TAG, "Ready for data");
        ...
        private void redirectToPcap() throws IOException {
            AppLogger.d(TAG, "Redirecting to PCAP: " + mFname);
        ...
        public void sendChunk(byte []data) {
            try {
                ...
                //AppLogger.d(TAG, "+CHUNK [" + data.length + "]");
            ...
            try {
                ...
                synchronized(this) {
                    if(mClients.size() >= MAX_CLIENTS) {
                        AppLogger.w(TAG, "Clients limit reached");
                        Utils.safeClose(client);
                        continue;
                    }
                }

                AppLogger.i(TAG, "New client: " + client.getInetAddress().getHostAddress() + ":" + client.getPort());
                ...
                if(!mRunning)
                    AppLogger.d(TAG, "Got termination request");
                else
                    AppLogger.d(TAG, e.getLocalizedMessage());
            }
        }
    ...
    public void stopDumper() throws IOException {
        ...
        while((mThread != null) && (mThread.isAlive())) {
            try {
                AppLogger.d(TAG, "Joining HTTP thread...");
                mThread.join();
            } catch (InterruptedException ignored) {}
        }
    }
    ...
    public void dumpData(byte[] data) throws IOException {
        synchronized(this) {
            ...
            /*while(it.hasNext()) {
                ...
                if(client.isClosed()) {
                   // it.remove();
                    AppLogger.d(TAG, "Client closed, active clients: " + mClients.size());
                }
            }*/
            //new
