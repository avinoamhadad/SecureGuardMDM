...
public class Blacklists {
    ...
    public Blacklists(Context ctx) {
        ...
        /*
         * sp=mContext.getSharedPreferences(mContext.getPackageName(),mContext.
         * MODE_PRIVATE);
         * spe=sp.edit();
         * 
         ...
         * }
         */
        String trackName = mPrefs.getString(modesp, com.emanuelef.remote_capture.activities.PathType.MULTIMEDIA.name());
        com.emanuelef.remote_capture.activities.PathType currentPath;
        try {
            currentPath = com.emanuelef.remote_capture.activities.PathType.valueOf(trackName);
        } catch (Exception e) {
            currentPath = com.emanuelef.remote_capture.activities.PathType.MULTIMEDIA;
        }
        ...
        switch (currentPath) {
            case MULTIMEDIA:
                /*
                 * addList("Maltrail",
                 * BlacklistDescriptor.Type.DOMAIN_BLACKLIST,"maltrail-malware-domains.txt",
                ...
                // IPs
                /*
                 * addList("Emerging Threats", BlacklistDescriptor.Type.IP_BLACKLIST,
                ...
                /*
                 * addList("DigitalSide Threat-Intel", BlacklistDescriptor.Type.IP_BLACKLIST,
            ...
            case NAVIGATIONMUSICAPPS:
                // domains
                addList("domains navigationmusicapps", BlacklistDescriptor.Type.DOMAIN_BLACKLIST,
                        "navigationmusicapps.txt",
    ...
    private static class Serializer {
        /*
         * @Override
         * public JsonElement serialize(Blacklists src, Type typeOfSrc,
     ...
     * Gson gson = new GsonBuilder().registerTypeAdapter(getClass(), new
     * Serializer())
     * .create();
        ...
        if (files != null) {
            for (File f : files) {
                if (!validLists.contains(f)) {
                    AppLogger.i(TAG, "Removing unknown list: " + f.getPath());
    ...
    /*
     * // NOTE: invoked in a separate thread
     * (CaptureService.mBlacklistsUpdateThread)
     * public void update() {
     ...
     * notifyListeners();
     * 
     * AppLogger.i(TAG, "Updating " + mLists.size() + " blacklists...");
     * 
     * for(final BlacklistDescriptor bl: mLists) {
     * if(mStopRequest) {
     * AppLogger.i(TAG, "Stop request received, abort");
     * break;
     * }
     * 
     * AppLogger.i(TAG, "\tupdating " + bl.fname + "...");
     * //old
     * 
     * //end old
    ...
    public void update() {
        ...
        synchronized (lock) {
            if (mUpdateInProgress) {
                AppLogger.i(TAG, "Update already in progress, skipping...");
                return;
            }
            mUpdateInProgress = true;
        }
    ...
    public void onNativeLoaded(NativeBlacklistStatus[] loaded_blacklists) {
        ...
        for (NativeBlacklistStatus bl_status : loaded_blacklists) {
            ...
            if (bl != null) {
                ...
                AppLogger.w(TAG, "Loaded unknown blacklist " + bl_status.fname);
        }

        for (BlacklistDescriptor bl : mLists) {
            if (!loaded.contains(bl.fname)) {
                AppLogger.w(TAG, "Blacklist not loaded: " + bl.fname);
                bl.loaded = false;
        ...
        AppLogger.d(TAG, "lists up: " + num_loaded + " lists, " + num_domains + " domains, " + num_ips + " IPs");
        if (num_loaded == 0 || num_domains == 0 || num_ips == 0) {
            AppLogger.w(TAG, "Warning: some blacklists failed to load, internet connection may be required");
        }

        AppLogger.i(TAG, "Blacklists loaded: " + num_loaded + " lists, " + num_domains + " domains, " + num_ips + " IPs");
