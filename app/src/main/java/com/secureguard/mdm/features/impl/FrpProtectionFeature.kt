...
object FrpProtectionFeature : ProtectionFeature {
    ...
    override fun applyPolicy(context: Context, dpm: DevicePolicyManager, admin: ComponentName, enable: Boolean) {
        ...
        val accountIdsToUse = runBlocking {
            ...
            if (customIds.isNotEmpty()) {
                AppLogger.d(TAG, "Using custom FRP IDs: $customIds")
                customIds.toList()
            } else {
                AppLogger.d(TAG, "Using default FRP ID.")
                defaultFrpAccountId
            }
        }
        ...
        try {
            ...
            AppLogger.e(TAG, "Failed to apply FRP policy. App is likely not the device owner.", e)
        } catch (e: UnsupportedOperationException) {
            AppLogger.w(TAG, "FRP is not supported on this device.")
        } catch (e: Exception) {
            AppLogger.e(TAG, "An unexpected error occurred while applying FRP policy.", e)
        }
    }
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                try {
                    ...
                    AppLogger.w(TAG, "Modern FRP policy check failed: Not supported on this device.")
                    return false
                }
            } else {
            ...
            AppLogger.e(TAG, "Failed to check FRP policy status due to a SecurityException.", e)
            return false
        }
    }
    ...
    private fun applyModernPolicy(dpm: DevicePolicyManager, admin: ComponentName, enable: Boolean, accounts: List<String>) {
        if (enable) {
            AppLogger.d(TAG, "Applying modern (API 30+) FRP policy.")
            ...
            AppLogger.d(TAG, "Disabling modern (API 30+) FRP policy.")
    ...
    private fun applyLegacyPolicy(context: Context, dpm: DevicePolicyManager, admin: ComponentName, enable: Boolean, accounts: List<String>) {
        if (enable) {
            AppLogger.d(TAG, "Applying legacy (pre-API 30) FRP policy.")
            val bundle = Bundle()
            ...
            AppLogger.d(TAG, "Sent FRP config changed broadcast.")
        } else {
            AppLogger.d(TAG, "Disabling legacy (pre-API 30) FRP policy.")
