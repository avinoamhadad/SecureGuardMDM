...
object BlockIncomingCallsFeature : ProtectionFeature {
    ...
    override fun applyPolicy(context: Context, dpm: DevicePolicyManager, admin: ComponentName, enable: Boolean) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            AppLogger.w(TAG, "Cannot apply policy, API level ${Build.VERSION.SDK_INT} is below required ${Build.VERSION_CODES.M}")
            return
        }
    ...
    private fun enableBlocking(context: Context, dpm: DevicePolicyManager, admin: ComponentName, repo: SettingsRepository) {
        if (isPolicyActive(context, dpm, admin)) {
            AppLogger.d(TAG, "enableBlocking called but NoPhone is already installed. Doing nothing.")
            return
        }

        val telecomManager = context.getSystemService(Context.TELECOM_SERVICE) as TelecomManager
        val originalDialer = telecomManager.defaultDialerPackage
        AppLogger.d(TAG, "Original dialer is: $originalDialer")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            if (originalDialer != null && originalDialer != NO_PHONE_PACKAGE_NAME) {
                ...
                try {
                    ...
                    AppLogger.d(TAG, "Successfully hid original dialer: $originalDialer")
                    installNoPhoneApp(context)
                } catch (e: Exception) {
                    AppLogger.e(TAG, "Failed to hide original dialer", e)
                }
            } else {
                AppLogger.w(TAG, "No original dialer found or it's already the nophone app. Trying to install nophone anyway.")
                installNoPhoneApp(context)
            }
        }
    }
    ...
    private fun disableBlocking(context: Context, dpm: DevicePolicyManager, admin: ComponentName, repo: SettingsRepository) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (originalDialer == null) {
                AppLogger.d(TAG, "disableBlocking called but no original dialer is saved. Doing nothing.")
                return@launch
            }
            ...
            try {
                ...
                AppLogger.d(TAG, "Successfully un-hid original dialer: $originalDialer")
                repo.setOriginalDialerPackage(null)
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to un-hide original dialer", e)
            }
        }
    }
    ...
    private fun installNoPhoneApp(context: Context) {
        ...
        try {
            ...
            session.close()
            AppLogger.d(TAG, "NoPhone installation session committed.")

        } catch (e: Exception) {
            AppLogger.e(TAG, "Failed to install NoPhone app from assets.", e)
        }
    }
