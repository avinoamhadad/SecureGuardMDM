...
class KioskViewModel @Inject constructor(
    ...
    data class UiState(
        ...
        val isAstoreEnabled: Boolean = false
    )
    ...
    sealed class SideEffect {
        ...
        object NavigateToAstore : SideEffect()
    ...
    init {
        ...
        viewModelScope.launch {
            settingsRepository.getAstoreEnabledFlow().collect { enabled ->
                _uiState.update { it.copy(isAstoreEnabled = enabled) }
            }
        }
    }
    ...
    fun verifyPassword(password: String) {
        viewModelScope.launch {
            if (passwordManager.verifyPassword(password)) {
                ...
                _sideEffect.emit(SideEffect.ShowToast(context.getString(R.string.kiosk_toast_wrong_password)))
            }
        }
    }
    ...
    fun onSettingsClick() {
        viewModelScope.launch {
            _sideEffect.emit(SideEffect.NavigateToKioskManagement)
        }
    }

    fun onAstoreClick() {
        viewModelScope.launch {
            _sideEffect.emit(SideEffect.NavigateToAstore)
        }
    }
