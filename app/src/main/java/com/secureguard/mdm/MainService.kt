...
class MainService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        AppLogger.d("MainService", "Service started.")
        startForeground(NOTIFICATION_ID, createNotification())

        serviceScope.launch {
            // --- השינוי כאן: משתמשים ברשימה מה-Registry ---
            val tasksToRun = bootTaskRegistry.allBootTasks
            AppLogger.d("MainService", "Executing ${tasksToRun.size} boot tasks from registry.")
            tasksToRun.forEach { task ->
                try {
                    task.onBootCompleted()
                } catch (e: Exception) {
                    AppLogger.e("MainService", "Error executing boot task: ${task::class.simpleName}", e)
                }
            }
            AppLogger.d("MainService", "All boot tasks executed. Stopping service.")
            stopSelf()
        }
    ...
    override fun onDestroy() {
        super.onDestroy()
        serviceJob.cancel()
        AppLogger.d("MainService", "Service destroyed.")
    }
}